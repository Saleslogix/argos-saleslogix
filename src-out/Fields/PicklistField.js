...
define('crm/Fields/PicklistField', [
    ...
    '../Views/PickList',
    'argos/FieldManager'
], function (declare, lang, string, LookupField, PickList, FieldManager) {
    var viewsByName = {}, getOrCreateViewFor, control, viewsByNameCount = 0;
    ...
    control = declare('crm.Fields.PicklistField', [LookupField], {
        ...
        createSelections: function () {
            var value = this.getText(), selections = (value)
                ? (value.indexOf(', ') !== -1)
                    ? value.split(', ')
                    : [value]
                : [];
        ...
        navigateToListView: function () {
            ...
            var options = this.createNavigationOptions(), view = App.getView(this.view) || getOrCreateViewFor(this.picklist);
            if (view && options) {
                view.show(options);
            }
        }
