define("crm/Integrations/BOE/PricingAvailabilityService", ["exports", "dojo/_base/lang", "argos/Dialogs/BusyIndicator", "argos/I18n"], function (_exports, _lang, _BusyIndicator, _I18n) {
  "use strict";
   ...
   */
  var resource = (0, _I18n["default"])('pricingAvailabilityService');
  ...
  var __class = _lang["default"].setObject('crm.Integrations.BOE.PricingAvailabilityService', {
    ...
    getOrderItemPricing: function getOrderItemPricing(orderItem, order, product, quantity, slxLocation, unitOfMeasure) {
      var options = {
        ...
        requestOptions: {
          childEntityIds: product ? product.$key : orderItem.Product ? orderItem.Product.$key : null,
          // eslint-disable-line
          childEntityName: 'Product',
          itemEntityName: 'SalesOrderItem',
          entityName: 'SalesOrder',
          entityId: order ? order.$key : orderItem.SalesOrder ? orderItem.SalesOrder.$key : null,
          // eslint-disable-line
          quantity: quantity ? quantity : orderItem.Quantity || 1,
          unitOfMeasureId: unitOfMeasure ? unitOfMeasure.$key : orderItem.UnitOfMeasure ? orderItem.UnitOfMeasure.$key : null,
          // eslint-disable-line
          slxLocationId: slxLocation ? slxLocation.$key : orderItem.SlxLocation ? orderItem.SlxLocation.$key : null,
          // eslint-disable-line
    ...
    getQuoteItemPricing: function getQuoteItemPricing(quoteItem, quote, product, quantity, slxLocation, unitOfMeasure) {
      var options = {
        ...
        requestOptions: {
          childEntityIds: product ? product.$key : quoteItem.Product ? quoteItem.Product.$key : null,
          // eslint-disable-line
          childEntityName: 'Product',
          ...
          unitOfMeasureId: unitOfMeasure ? unitOfMeasure.$key : quoteItem.UnitOfMeasure ? quoteItem.UnitOfMeasure.$key : null,
          // eslint-disable-line
          slxLocationId: slxLocation ? slxLocation.$key : quoteItem.SlxLocation ? quoteItem.SlxLocation.$key : null,
          // eslint-disable-line
            ...
            for (var prop in item.Properties) {
              ...
              if (toDouble) {
                ...
                if (value.search('#') !== -1) {
                  ...
                  value = values[1];
                  data["".concat(dataItem.propertyName, "Code")] = {
                    propertyName: "".concat(dataItem.propertyName, "Code"),
                    type: 'text',
                    value: values[0]
                  };
                  dataItem.propertyName = "".concat(dataItem.propertyName, "Id");
                }
              ...
              for (var key in product.Properties) {
                if (key !== 'ProductCode') {
                  if (key.indexOf('.') > 0 && key.indexOf('ErpExtId') > 0) {
                    ...
                    entity["".concat(propName[0], "ID")] = extId[1];
                  } else {
