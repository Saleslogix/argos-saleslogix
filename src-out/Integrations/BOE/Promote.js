define("crm/Integrations/BOE/Promote", ["exports", "dojo/_base/declare", "dojo/_base/lang", "dojo/string", "dojo/when", "argos/Models/Adapter", "argos/Dialogs/BusyIndicator", "dojo/Deferred", "argos/Dropdown", "dijit/_Widget", "argos/_Templated", "./Models/Names", "argos/I18n"], function (_exports, _declare, _lang, _string, _when, _Adapter, _BusyIndicator, _Deferred, _Dropdown, _Widget2, _Templated2, _Names, _I18n) {
  "use strict";
   ...
   */
  var resource = (0, _I18n["default"])('promote');
  ...
  var __class = (0, _declare["default"])('crm.Integrations.BOE.Promote', [_Widget2["default"], _Templated2["default"]], {
    ...
    initBackOfficeModel: function initBackOfficeModel() {
      var model = this.getModel(_Names["default"].BACKOFFICE);
    ...
    initAccountingEntitiesModel: function initAccountingEntitiesModel() {
      var model = this.getModel(_Names["default"].BACKOFFICEACCOUNTINGENTITY);
    ...
    getAccountingEntitiesEntries: function getAccountingEntitiesEntries(backOfficeKey) {
      if (backOfficeKey) {
        if (this._backOfficeEntries.length) {
          var query = this._accountingEntityModel.getEntries("BackOffice.$key eq \"".concat(backOfficeKey, "\""), {
            returnQueryResults: true,
            queryModelName: 'list'
          });
      ...
      App.modal.createSimpleDialog({
        title: 'alert',
        content: this.noBackOfficesText
      });
    },
    initIntegrationMappingModel: function initIntegrationMappingModel() {
      var model = this.getModel(_Names["default"].OPERATINGCOMPANY);
      ...
      if (options && scope) {
        ...
        request.execute(entry, {
          ...
          failure: function failure(err) {
            App.toast.add({
              title: 'Error',
              message: _string["default"].substitute(_this.errorMessage, {
    ...
    createAlertDialog: function createAlertDialog(err) {
      ...
      App.modal.showToolbar = true;
      App.modal.resolveDeferred(true);
      return App.modal.createSimpleDialog({
        title: 'alert',
        content: err,
    ...
    createAccountingDropdown: function createAccountingDropdown() {
      ...
      this._accountingDropdown = new _Dropdown["default"]({
        id: 'accountingDropdown',
        dropdownClass: 'promote__dropdown'
      });

      this._accountingDropdown.createList({
        items: this._accountingSelections,
        defaultValue: this._accountingSelections[0] ? this._accountingSelections[0].value : ''
      }); // TODO: change the defaultValue selected
    ...
    createBackOfficeDropdown: function createBackOfficeDropdown() {
      if (!this._backOfficeDropdown) {
        this._backOfficeDropdown = new _Dropdown["default"]({
          id: 'backOfficeDropdown',
          dropdownClass: 'promote__dropdown',
          onSelect: this.updateAccountingDropdown,
          onSelectScope: this
        });

        this._backOfficeDropdown.createList({
          items: this._backOfficeSelections,
          defaultValue: this._backOfficeSelections[0] ? this._backOfficeSelections[0].value : ''
        }); // TODO: change the defaultValue selected
    ...
    getAccountingSystem: function getAccountingSystem() {
      if (!this._busy) {
        this._busy = new _BusyIndicator["default"]({
          id: "".concat(this.id, "__busyIndicator")
        });
        this._accountingBusy = new _BusyIndicator["default"]({
          id: "".concat(this.id, "__busyIndicator__accounting"),
          size: 'small',
          label: null,
          containerClass: 'busyField'
        });
