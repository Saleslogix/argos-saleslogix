...
define('crm/Views/Calendar/DayView', [
    ...
    'moment'
], function (declare, lang, string, query, domClass, domConstruct, ErrorManager, convert, List, _LegacySDataListMixin, moment) {
    var __class = declare('crm.Views.Calendar.DayView', [List, _LegacySDataListMixin], {
        ...
        createEventRequest: function () {
            var eventSelect = this.eventQuerySelect, eventWhere = this.getEventQuery(), request = new Sage.SData.Client.SDataResourceCollectionRequest(this.getService())
        ...
        formatQueryForActivities: function () {
            var queryWhere, startDate, endDate;
            queryWhere = [
            ...
            return string.substitute(queryWhere, [App.context['user'] && App.context['user']['$key'],
        ...
        navigateToWeekView: function () {
            var view = App.getView(this.weekView), navDate = this.currentDate ? this.currentDate : moment().startOf('day'), options = { currentDate: navDate.valueOf() };
            view.show(options);
        },
        navigateToMonthView: function () {
            var view = App.getView(this.monthView), navDate = this.currentDate ? this.currentDate : moment().startOf('day'), options = { currentDate: navDate.valueOf() };
            view.show(options);
        },
