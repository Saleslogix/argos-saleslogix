...
define('crm/Views/Calendar/WeekView', [
    ...
    'crm/Format',
    'moment'
], function (declare, lang, query, string, domConstruct, domClass, ErrorManager, convert, List, _LegacySDataListMixin, format, moment) {
    var __class = declare('crm.Views.Calendar.WeekView', [List, _LegacySDataListMixin], {
        ...
        setWeekTitle: function () {
            var start = this.getStartDay(this.currentDate), end = this.getEndDay(this.currentDate);
        ...
        processFeed: function (feed) {
            this.feed = feed;
            var todayNode = this.addTodayDom(), entryGroups = this.entryGroups, feedLength = feed['$resources'].length, entryOrder = [], dateCompareString = 'YYYY-MM-DD', o = [], i, currentEntry, entryOrderLength, entryGroup, currentDateCompareKey, currentGroup, startDate;
        ...
        getEventQuery: function () {
            var startDate = this.weekStartDate, endDate = this.weekEndDate;
        ...
        processEventFeed: function (feed) {
            var o = [], i, event, feedLength = feed['$resources'].length;
            if (feedLength === 0) {
                this.hideEventList();
                return false;
            }
        ...
        activateEventMore: function () {
            var view = App.getView('event_related'), where = this.getEventQuery();
            if (view) {
        ...
        navigateToDayView: function () {
            var view = App.getView(this.activityListView), options = { currentDate: this.currentDate.toDate().valueOf() || moment().startOf('day').valueOf() };
            view.show(options);
        },
        navigateToMonthView: function () {
            var view = App.getView(this.monthView), options = { currentDate: this.currentDate.toDate().valueOf() || moment().startOf('day').valueOf() };
            view.show(options);
        },
