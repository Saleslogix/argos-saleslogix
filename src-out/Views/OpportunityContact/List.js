...
define('crm/Views/OpportunityContact/List', [
    ...
    var __class = declare('crm.Views.OpportunityContact.List', [List], {
        ...
        complete: function () {
            var view = App.getPrimaryActiveView(), context, selections, selectionKey, selectionModel = view && view.get('selectionModel'), entry;
            if (!selectionModel) {
                return;
            }
        ...
        navigateToInsertView: function (entry) {
            var view = App.getView(this.insertView), options = {
                entry: entry,
                insert: true
            };
            if (view && options) {
        ...
        navigateToSelectView: function () {
            var view = App.getView(this.selectView), options = this.createNavigationOptions();
            if (view && options) {
                view.show(options);
            }
        },
