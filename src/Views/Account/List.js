...
const __class = declare('crm.Views.Account.List', [List, _RightDrawerListMixin, _MetricListMixin, _CardLayoutListMixin, _GroupListMixin], {
  ...
  itemTemplate: new Simplate([
    ...
    '<p class="listview-subheading">{%: $.Industry %}</p>',
    '<p class="listview-subheading">',
    '{%: $$.joinFields(" | ", [$.Type, $.SubType]) %}',
    '</p>',
    '<p class="listview-subheading">{%: $.AccountManager && $.AccountManager.UserInfo ? $.AccountManager.UserInfo.UserName : "" %}',
    '{% if ($.Owner && $.Owner.OwnerDescription) { %} | {%: $.Owner.OwnerDescription %}{% } %}</p>',
    '<p class="listview-subheading">{%: $.WebAddress %}</p>',
    '{% if ($.MainPhone) { %}',
    '<p class="listview-subheading">',
  ...
  groupLayoutItemTemplate: new Simplate([
    ...
    '<p class="listview-subheading"><span class="group-label">{%= $$.getGroupFieldLabelByName($,"MainPhone") %} </span><span class="group-entry">{%= $$.getGroupFieldValueByName($, "MainPhone") %}</span></p>',
    '</div><div style="float:left;">',
    '<p class="listview-subheading"><span class="group-label">{%= $$.getGroupFieldLabelByName($,"Status") %} </span><span class="group-entry">{%= $$.getGroupFieldValueByName($, "Status") %}</span></p>',
    '<p class="listview-subheading"><span class="group-label">{%= $$.getGroupFieldLabelByName($,"Type") %} </span><span class="group-entry">{%= $$.getGroupFieldValueByName($, "Type") %}</span></p>',
    '</div>',
  ]),
