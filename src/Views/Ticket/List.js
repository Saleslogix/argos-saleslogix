...
const __class = declare('crm.Views.Ticket.List', [List, _RightDrawerListMixin, _MetricListMixin, _CardLayoutListMixin, _GroupListMixin], {
  ...
  itemTemplate: new Simplate([
    ...
    '<p class="listview-subheading">{%: $.Subject %}</p>',
    '{% if(($.Account) && (!$.Contact)) { %}',
    '<p class="listview-subheading">{%: $$.viewContactActionText + ": " + $.Account.AccountName %}</p>',
    '{% } %}',
    '{% if(($.Account) && ($.Contact)) { %}',
    '<p class="listview-subheading">{%: $$.viewContactActionText + ": " + $.Contact.NameLF + " | " + $.Account.AccountName %}</p>',
    '{% } %}',
    '<p class="listview-subheading"> {%: $.AssignedTo ? ($$.assignedToText + $.AssignedTo.OwnerDescription) : this.notAssignedText %}</p>',
    '{% if($.Urgency) { %}',
    '<p class="listview-subheading">{%: $$.urgencyText + $.Urgency.Description %}</p>',
    '{% } %}',
    '{% if($.Area) { %}',
    '<p class="listview-subheading">{%: $$._areaCategoryIssueText($) %}</p>',
    '{% } %}',
    '{% if($.CreateDate) { %}',
    '<p class="listview-subheading">{%: $$.createdOnText %}  {%: crm.Format.relativeDate($.CreateDate) %}</p>',
    '{% } %}',
    '{% if($.ModifyDate) { %}',
    '<p class="listview-subheading">{%: $$.modifiedText %}  {%: crm.Format.relativeDate($.ModifyDate) %}</p>',
    '{% } %}',
    '{% if($.NeededByDate) { %}',
    '<p class="listview-subheading">{%: $$.neededByText %}  {%: crm.Format.relativeDate($.NeededByDate) %}</p>',
    '{% } %}',
  ]),
