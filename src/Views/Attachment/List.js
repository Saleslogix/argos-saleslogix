...
const __class = declare('crm.Views.Attachment.List', [List, _RightDrawerListMixin, _CardLayoutListMixin, _LegacySDataListMixin], {
  ...
  fileTemplate: new Simplate([
    ...
    '<p class="listview-subheading"><span>({%: $$.uploadedOnText %} {%: crm.Format.relativeDate($.attachDate) %})&nbsp;</span>',
    '<span>{%: crm.Format.fileSize($.fileSize) %} </span></p>',
    '<p class="listview-subheading"><span>{%: crm.Utility.getFileExtension($.fileName) %} </span></p>',
    '{% if($.user) { %}',
    '<p class="listview-subheading"><span>{%: $.user.$descriptor  %}</span></p>',
    '{% } %}',
  ]),
  urlTemplate: new Simplate([
    '<p class="listview-subheading"><span>{%: $.description %} &nbsp;</span></p>',
    '{% if ($.attachDate) { %}',
    '<p class="listview-subheading"><span>({%: $$.uploadedOnText %} {%: crm.Format.relativeDate($.attachDate) %})&nbsp;</span></p>',
    '{% } %}',
    '<p class="listview-subheading"><span>{%: $.url %}&nbsp;</span></p>',
    '{% if($.user) { %}',
    '<p class="listview-subheading"><span>{%: $.user.$descriptor  %}</span></p>',
    '{% } %}',
  ]),
