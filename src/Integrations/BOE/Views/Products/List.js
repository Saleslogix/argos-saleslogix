...
const __class = declare('crm.Integrations.BOE.Views.Products.List', [List], {
  ...
  itemTemplate: new Simplate([
    ...
    '{% } %}',
    '{% if ($.Family) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.familyText %}</label> {%: $.Family %}</p>',
    '{% } %}',
    '{% if ($.Status) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.statusText %}</label> {%: $.Status %}</p>',
    '{% } %}',
    '{% if ($.Price) { %}',
