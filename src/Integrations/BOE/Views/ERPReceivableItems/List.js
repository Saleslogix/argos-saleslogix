...
const __class = declare('crm.Integrations.BOE.Views.ERPReceivableItems.List', [List, _RightDrawerListMixin, _MetricListMixin, _CardLayoutListMixin], {
  ...
  itemTemplate: new Simplate([
    ...
    '<p class="listview-subheading"><label class="group-label">{%: $$.receivablesIdText %}</label> {%: $.ErpReceivable.ErpExtId %}</p>',
    '{% if ($.ErpInvoice && $.ErpInvoice.ErpExtId) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.invoiceIDText %}</label> {%: $.ErpInvoice.ErpExtId %}</p>',
    '{% } %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.productNameText %}</label> {%: $.ProductName %}</p>',
    '{% if ($.ErpLineTotalAmount) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.lineTotalText %}</label> ',
