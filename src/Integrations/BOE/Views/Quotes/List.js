...
const __class = declare('crm.Integrations.BOE.Views.Quotes.List', [List, _RightDrawerListMixin, _MetricListMixin, _CardLayoutListMixin, _GroupListMixin], {
  ...
  itemTemplate: new Simplate([
    '<p class="listview-subheading"><label class="group-label">{%: $$.quoteNumberText %}</label> {%: $.QuoteNumber %}</p>',
    '{% if ($.Account && $.Account.AccountName) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.accountText %}</label> {%: $.Account.AccountName %}</p>',
    '{% } %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.createDateText %}</label> {%: $$.formatter.date($.CreateDate) %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.grandTotalLabelText %} </label>',
    '{%: $$.util.formatMultiCurrency($.DocGrandTotal, $.CurrencyCode) %}',
    '</p>',
    '{% if ($.ErpExtId) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.erpStatusLabelText %}</label> {%: $$.formatErpStatus($.ErpStatus) %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.documentDateText %}</label> {%: $$.formatter.date($.DocumentDate) %}</p>',
    '{% } else { %}',
