...
const __class = declare('crm.Integrations.BOE.Views.SalesOrders.List', [List, _RightDrawerListMixin, _MetricListMixin, _CardLayoutListMixin, _GroupListMixin], {
  ...
  itemTemplate: new Simplate([
    ...
    '<p class="listview-subheading"><label class="group-label">{%: $$.erpOrderIdText %}</label> {%: $.ErpExtId %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.customerPONumberText %}</label> {%: $.CustomerPurchaseOrderNumber %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.accountText %}</label> {%: $.Account.AccountName %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.orderDateText %}</label> {%: $$.formatter.date($.OrderDate) %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.baseGrandTotalText %}</label> ',
    '{%: $$.util.formatMultiCurrency($.GrandTotal, $.BaseCurrencyCode) %}',
    '</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.grandTotalText %}</label> ',
    '{%: $$.util.formatMultiCurrency($.DocGrandTotal, $.CurrencyCode) %}',
    '</p>',
    '{% if ($.ErpExtId) { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.erpStatusLabelText %}</label> {%: $$.formatErpStatus($.ERPSalesOrder.ERPStatus) %}</p>',
    '<p class="listview-subheading"><label class="group-label">{%: $$.documentDateText %}</label> {%: $$.formatter.date($.ErpDocumentDate) %}</p>',
    '{% } else { %}',
    '<p class="listview-subheading"><label class="group-label">{%: $$.statusLabelText %}</label> {%: $.Status %}</p>',
    '{% } %}',
  ]),
